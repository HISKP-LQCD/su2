...
void gradient_flow(const gaugeconfig<Group> &U,
  ...
  P[2] = gauge_energy(U) / den_common;
  P_ss[2] = gauge_energy(U, true) / den_common;
  energy_density(U, density, topQ);
  E[2] = density;
  energy_density(U, density_ss, topQ_ss, true, true);
  E_ss[2] = density_ss;
  ...
  while (t[1] < tmax - eps) {
    ...
    for (unsigned int x0 = 1; x0 < 3; x0++) {
      ...
      P[x0] = gauge_energy(Vt) / den_common;
      P_ss[x0] = gauge_energy(Vt, true) / den_common;
      energy_density(Vt, density, topQ, true);
      E[x0] = density;
      Q[x0] = topQ;
      energy_density(Vt, density_ss, topQ_ss, true, true);
      E_ss[x0] = density_ss;
    }
