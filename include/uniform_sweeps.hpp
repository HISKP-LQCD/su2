...
// apply sweeps drawing elements uniformly with the constraint of P > Pmin
// the sweeps are done until:
// the configuration changes AND has been accepted at least "n_sweeps_min" times
                      ...
                      const size_t &n_sweeps_min)
{
  ...
  size_t i_sweep = 0;
  // while ((!changed) || (accepted < n_sweeps_min)) {
  for (size_t x0 = 0; x0 < U.getLt(); x0++)
  {
    for (size_t x1 = 0; x1 < U.getLx(); x1++)
    {
      for (size_t x2 = 0; x2 < U.getLy(); x2++)
      {
        ...
        {
          if (changed && (accepted >= n_sweeps_min))
          {
            break;
          }
