...
namespace u1 {
  ...
  class metropolis_algo : public program<Group, gp::metropolis_u1> {
  private:
  ...
  public:
    ...
    void do_sweep(const size_t &i, const size_t &inew) {
      if ((*this).sparams.do_mcmc) {
        ...
        rate += this->sweep((*this).pparams, (*this).U, engines, (*this).sparams.delta,
                            (*this).sparams.N_hit, (*this).pparams.beta,
    ...
    void save_acceptance_rates() {
      if ((*this).sparams.do_mcmc) {
        ...
        (*this).acceptancerates
          << rate[0] / double((*this).sparams.n_meas) << " "
          << rate[1] / double((*this).sparams.n_meas) << " " << (*this).pparams.beta
          << " " << (*this).pparams.Lx << " " << (*this).pparams.Lt << " "
          << (*this).pparams.xi << " " << (*this).sparams.delta << " "
          << (*this).sparams.heat << " " << (*this).threads << " "
          << (*this).sparams.N_hit << " " << (*this).sparams.n_meas << " "
    ...
    void run(const YAML::Node &nd) {
      ...
      for (size_t i = (*this).sparams.icounter;
        ...
        if ((*this).sparams.do_omeas && inew != 0 &&
            (inew % (*this).sparams.N_save) == 0) {
          this->do_omeas_i(i);
        }
      }
