...
parser = argparse.ArgumentParser(prog="retrace2", description="""format retrace2 files in the "hadron" format""")
...
def hadronize(name):
    ...
    df_new = pd.DataFrame(np.zeros(shape=(1, 2)),
                          columns=["i", "retrace2"], index=["remove"])
    ...
    for i_f in range(len(file_index)):
        ...
        df_i = pd.read_csv(path_i, sep=" ")["retrace2"]
        arr1 = df_i.to_numpy()
    ...
    if save_df_new:
        ...
        df_new.to_csv(outputfolder + "/"+name+f'{gaugemass: .2f}' + ".dat", sep=" ", index=False)
    print("Saving the new iconf file")
    if save_iconf:
        pd.DataFrame(sorted(file_index)).to_csv(
            outputfolder+f'{gaugemass : .2f}' + "iconfs.dat", header=False, sep=" ", index=False)
    ####
