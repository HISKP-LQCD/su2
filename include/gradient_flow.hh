                   ...
                   const bool &save_config)
{
  ...
  P[2] = retr_sum_Wplaquettes(U) / den_common;
  P_ss[2] = retr_sum_Wplaquettes(U, true) / den_common;
  leafs_and_Qtop(U, density, topQ);
  E[2] = density;
  leafs_and_Qtop(U, density_ss, topQ_ss, true, true);
  E_ss[2] = density_ss;
  ...
  while (t[1] < tmax - eps)
  {
    ...
    for (unsigned int x0 = 1; x0 < 3; x0++)
    {
      ...
      P[x0] = retr_sum_Wplaquettes(Vt) / den_common;
      P_ss[x0] = retr_sum_Wplaquettes(Vt, true) / den_common;
      leafs_and_Qtop(Vt, density, topQ, true);
      E[x0] = density;
      Q[x0] = topQ;
      leafs_and_Qtop(Vt, density_ss, topQ_ss, true, true);
      E_ss[x0] = density_ss;
    }
