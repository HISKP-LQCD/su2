...
void random_element(_su2 &U, URNG &engine, const double delta = 1.) {
  std::uniform_real_distribution<double> dist1(0, pi());
  std::uniform_real_distribution<double> dist2(0., 2 * pi());
  std::uniform_real_distribution<double> dist3(0., delta  * pi());
  const double psi = dist3(engine);
  const double theta = dist1(engine);
  const double phi = dist2(engine);

  //const double r = sqrt(1 - u * u);
  //const double salpha = sin(alpha);

  U = _su2(Complex(cos(psi), sin(psi) * cos(theta)), sin(psi) * sin(theta) * Complex(sin(phi), cos(phi)));
  return;
}
#endif
/**
