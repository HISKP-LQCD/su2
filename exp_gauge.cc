...
_su3 exp(const adjointsu3<double> &x) {
  ...
  // determinant of H
  const double detH =
    2.0 * sqrt(3.0) * std::pow(n[0], 2.0) * n[7] / 3.0 + 2.0 * n[0] * n[3] * n[5] +
    2.0 * n[0] * n[4] * n[6] + 2.0 * sqrt(3.0) * std::pow(n[1], 2.0) * n[7] / 3.0 -
    2.0 * n[1] * n[3] * n[6] + 2.0 * n[1] * n[4] * n[5] +
    2.0 * sqrt(3.0) * std::pow(n[2], 2.0) * n[7] / 3.0 + n[2] * std::pow(n[3], 2.0) +
    n[2] * std::pow(n[4], 2.0) - n[2] * std::pow(n[5], 2.0) - n[2] * std::pow(n[6], 2.0) -
    sqrt(3.0) * std::pow(n[3], 2.0) * n[7] / 3.0 -
    sqrt(3.0) * std::pow(n[4], 2.0) * n[7] / 3.0 -
    sqrt(3.0) * std::pow(n[5], 2.0) * n[7] / 3.0 -
    sqrt(3.0) * std::pow(n[6], 2.0) * n[7] / 3.0 -
    2.0 * sqrt(3.0) * std::pow(n[7], 3.0 / 9.0);
  const double phi = (1.0 / 3.0) * (acos((3.0 / 2.0) * sqrt(3.0) * detH) - M_PI / 2.0);

  const Complex I(0.0, 1.0); // imaginary unit

  // 1st and 2nd row of H
  const std::array<Complex, 3> H_1 = {n[2] + sqrt(3.0) * n[7] / 3.0, n[0] - I * n[1],
                                      n[3] - I * n[4]};
  const std::array<Complex, 3> H_2 = {n[0] + I * n[1], -n[2] + sqrt(3.0) * n[7] / 3.0,
                                      n[5] - I * n[6]};

  // 1st and 2nd row of H^2
  const std::array<Complex, 3> H2_1 = {
    (n[0] - I * n[1]) * (n[0] + I * n[1]) + std::pow(n[2] + sqrt(3.0) * n[7] / 3.0, 2.0) +
      (n[3] - I * n[4]) * (n[3] + I * n[4]),
    (n[0] - I * n[1]) * (-n[2] + sqrt(3.0) * n[7] / 3.0) +
      (n[0] - I * n[1]) * (n[2] + sqrt(3.0) * n[7] / 3.0) +
      (n[3] - I * n[4]) * (n[5] + I * n[6]),
    -2.0 * sqrt(3.0) * n[7] * (n[3] - I * n[4]) / 3.0 +
      (n[0] - I * n[1]) * (n[5] - I * n[6]) +
      (n[2] + sqrt(3.0) * n[7] / 3.0) * (n[3] - I * n[4])};
  const std::array<Complex, 3> H2_2 = {
    (n[0] + I * n[1]) * (-n[2] + sqrt(3.0) * n[7] / 3.0) +
      (n[0] + I * n[1]) * (n[2] + sqrt(3.0) * n[7] / 3.0) +
      (n[3] + I * n[4]) * (n[5] - I * n[6]),
    (n[0] - I * n[1]) * (n[0] + I * n[1]) +
      std::pow(-n[2] + sqrt(3.0) * n[7] / 3.0, 2.0) +
      (n[5] - I * n[6]) * (n[5] + I * n[6]),
    -2.0 * sqrt(3.0) * n[7] * (n[5] - I * n[6]) / 3.0 +
      (n[0] + I * n[1]) * (n[3] - I * n[4]) +
  ...
  for (size_t k = 0; k <= 2; k++) {
    ...
    for (size_t i = 0; i < 3; i++) {
      const Complex u_ik = H2_1[i] + (2.0 / sqrt(3.0)) * H_1[i] * sin(pk) -
