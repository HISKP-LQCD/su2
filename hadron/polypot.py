...
parser = argparse.ArgumentParser(
    prog='polypot',
    description="""format polypot_loop files in the "hadron" format""")
...
#rmax = nd["polypot"]["rmax"]
nd_omeas = nd["omeas"]
resdir = nd_omeas["res_dir"]
nd_polypot = nd_omeas["polypot"]
subdir = nd_polypot["subdir"]
rmax = nd_polypot["rmax"]
...
def hadronize(name):
    ...
    column_names = ["i"]
    loop_index = 0
    while loop_index < rmax:
        column_names.append(str(loop_index + 1))
        loop_index += 1
    
    df_new = pd.DataFrame(np.zeros(shape=(1, rmax+1)),
                          columns=column_names, index=["remove"])
    ...
    for i_f in range(len(file_index)):
        ...
        df_i = pd.read_csv(path_i, sep=" ", header = 0)[column_names[1:]]
        arr1 = df_i.to_numpy()
        print(arr1)
