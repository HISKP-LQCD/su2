...
public:
  ...
  explicit _partitionsu2(size_t m) : m(m) {
    j[0] = m;
    j[1] = 0;
    j[2] = 0;
    j[3] = 0;
  ...
  explicit _partitionsu2(size_t _m,
                 size_t * _j,
                 int * _s){
    for(int i = 0; i < 4; i++) {
      j[i] = _j[i];
  ...
  explicit _partitionsu2(size_t m,
  ...
  friend su2 operator*(const _partitionsu2 &U1, const _partitionsu2 &U2);
  friend su2 operator*(const su2 &U1, const _partitionsu2 &U2);
  friend su2 operator*(const _partitionsu2 &U1, const su2 &U2);
  ...
  void operator=(const _partitionsu2 &U) {
    for(int i = 0; i < 4; i++) {
      j[i] = U.getj(i);
      s[i] = U.gets(i);
      m = U.getm();
  ...
  _partitionsu2 dagger() const {
    return(_partitionsu2(m, j[0], j[1], j[2], s[0], -s[1], -s[2], -s[3]));
  }
  double trace() {
...
class _Gsu2 : private _partitionsu2 {
public:
  double trace() {
    return(2 * s[0] * sqrt(static_cast<double>(j[0])/static_cast<double>(m)));
  }
...
class _Osu2 : private _partitionsu2 {
public:
  double getJ() const {
