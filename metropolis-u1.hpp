...
namespace u1 {
  ...
  private:
    ...
    void create_directories() {
      boost::filesystem::create_directories(
        boost::filesystem::absolute(mcparams.conf_dir));
      boost::filesystem::create_directories(
    ...
    void save_acceptance_rates() {
      ...
      if (mcparams.do_mcmc) {
        std::cout << "## Acceptance rate "
                  << rate[0] / static_cast<double>(mcparams.n_meas)
        ...
        acceptancerates << rate[0] / static_cast<double>(mcparams.n_meas) << " "
                        ...
                        << " " << threads << " " << mcparams.N_hit << " "
                        << mcparams.n_meas << " " << mcparams.seed << " " << std::endl;
  ...
  public:
    ...
    void run(int ac, char *av[]) {
       ...
       * */
      for (size_t i = mcparams.icounter;
           i < mcparams.n_meas * threads + mcparams.icounter; i += threads) {
