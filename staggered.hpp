...
namespace staggered {
  ...
  template <class Float, class Type> class spinor_lat_4d {
  private:
    std::vector<Type> Psi;
  ...
  spinor_lat_4d<Float, Type> a_plus_lambda_b(const spinor_lat_4d<Float, Type> &a,
                                             const Type_lambda &lambda,
                                             const spinor_lat_4d<Float, Type> &b) {
    const int N = a.size();
  ...
  spinor_lat_4d<Float, Type> operator+(const spinor_lat_4d<Float, Type> &a,
                                       const spinor_lat_4d<Float, Type> &b) {
  ...
  template <class Float, class Type>
  spinor_lat_4d<Float, Type> gaussian_spinor_normalized(const size_t &n,
                                                        const Float &avr,
                                                        const Float &sigma,
                                                        const size_t &seed) {
  ...
  template <class Float, class Type>
  Type complex_dot_product(const spinor_lat_4d<Float, Type> &A,
                           const spinor_lat_4d<Float, Type> &B) {
    const int N = A.size();
    Type sum = 0.0;
  ...
  template <class Float, class Type>
  Type operator*(const spinor_lat_4d<Float, Type> &A,
                 const spinor_lat_4d<Float, Type> &B) {
  ...
  template <class Float, class Type> Float norm(const spinor_lat_4d<Float, Type> &psi) {
    return psi.norm();
  }
  ...
  public:
    ...
    spinor_lat_4d<Float, Type> inv(const spinor_lat_4d<Float, Type> &psi,
                                   const Float &tol,
                                   const size_t &verb,
      ...
      const LAvector phi0 =
        staggered::gaussian_spinor_normalized<Float, Complex>(N, 0.0, 1.0, seed);
  ...
  spinor_lat_4d<Float, Type> operator*(const DdagD_matrix_lat<Float, Type, Group> &M,
                                       const spinor_lat_4d<Float, Type> &psi) {
...
#pragma omp parallel for
          ...
          for (size_t x3 = 0; x3 < Lz; x3++) {
            ...
            for (size_t mu = 0; mu < nd; mu++) {
              ...
              for (size_t nu = 0; nu < nd; nu++) {
                ...
                phi(x, dims) -= fact2 * (*U)(xm, mu) * (*U)(xm, mu) * psi(xmm, dims);
                phi(x, dims) +=
                  fact2 * (*U)(xm, mu) * (*U)(xmm, mu).dagger() * psi(xmm, dims);
              ...
              //
              phi(x, dims) +=
                m * fact_mu * ((*U)(x, mu) + (*U)(x, mu).dagger()) * psi(xp, dims);
              phi(x, dims) -=
                m * fact_mu * ((*U)(xm, mu).dagger() + (*U)(xm, mu)) * psi(xm, dims);
  ...
  template <class Float, class Type, class Group>
  spinor_lat_4d<Float, Type>
  apply_D(gaugeconfig<Group> *U, const Float &m, const spinor_lat_4d<Float, Type> &psi) {
    const size_t Lt = U->getLt(), Lx = U->getLx(), Ly = U->getLy(), Lz = U->getLz();
    const std::vector<size_t> dims = {Lt, Lx, Ly, Lz}; // vector of dimensions
    const size_t nd = U->getndims();
...
#pragma omp parallel for
        ...
        for (size_t x2 = 0; x2 < Ly; x2++) {
          for (size_t x3 = 0; x3 < Lz; x3++) {
            ...
            for (size_t mu = 0; mu < nd; mu++) {
              ...
              phi(x, dims) +=
                +(1.0 / 2.0) * eta_x_mu * (*U)(x, mu) * psi(xp, dims) -
  ...
  template <class Float, class Type, class Group>
  spinor_lat_4d<Float, Type> apply_Ddag(gaugeconfig<Group> *U,
                                        const Float &m,
                                        const spinor_lat_4d<Float, Type> &psi) {
    const size_t Lt = U->getLt(), Lx = U->getLx(), Ly = U->getLy(), Lz = U->getLz();
    const std::vector<size_t> dims = {Lt, Lx, Ly, Lz}; // vector of dimensions
...
#pragma omp parallel for
        ...
        for (size_t x2 = 0; x2 < Ly; x2++) {
          for (size_t x3 = 0; x3 < Lz; x3++) {
            ...
            for (size_t mu = 0; mu < nd; mu++) {
              ...
              phi(x, dims) +=
                +(1.0 / 2.0) * eta_x_mu * (*U)(x, mu).dagger() * psi(xp, dims) -
  ...
  // returns the result of  dD*psi, where dD is the derivative of the Dirac operator for a
  // U(1) theory as in eq. 8.36 of Gattringer&Lang
  template <class Float, class Type, class Group>
  spinor_lat_4d<Float, Type> apply_der_D(const std::vector<size_t> &x,
                                         const size_t &mu,
                                         gaugeconfig<Group> *U,
                                         const Float &m,
                                         const spinor_lat_4d<Float, Type> &psi) {
    ...
    phi(xm, dims) =
      -(1.0 / 2.0) * eta_xm_mu * (-i) * (*U)(x, mu).dagger() * psi(xm, dims);

    return phi;
  }

  // returns the reslut of  dD^{\dagger}*psi, where dD^{\dagger} is the derivative of the
  // Dirac operator (daggered) for a U(1) theory analogously to in eq. 8.36 of
  // Gattringer&Lang
  template <class Float, class Type, class Group>
  spinor_lat_4d<Float, Type> apply_der_Ddag(const std::vector<size_t> &x,
                                            const size_t &mu,
                                            gaugeconfig<Group> *U,
                                            const Float &m,
                                            const spinor_lat_4d<Float, Type> &psi) {
